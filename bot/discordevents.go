...
func InvalidateCache(guildID, userID int64) {
	if userID != 0 {
		common.RedisPool.Do(radix.Cmd("DEL", common.CacheKeyPrefix+discordgo.StrID(userID)+":guilds"))
	}
	if guildID != 0 {
		common.RedisPool.Do(radix.Cmd("DEL", common.CacheKeyPrefix+common.KeyGuild(guildID)))
		common.RedisPool.Do(radix.Cmd("DEL", common.CacheKeyPrefix+common.KeyGuildChannels(guildID)))
	}
}
